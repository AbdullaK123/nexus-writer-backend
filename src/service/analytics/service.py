...
class AnalyticsService:
    ...
    def _get_cache_key(
        ...
        user_id: str,
        extraction: Literal[
            "plot_threads", "act_segmentation", "contradictions", "entities"
        ],
    ) -> str:
    ...
    async def get_prompt_inputs(
        ...
        match lense:
            ...
            case "plot":
                plot_threads, act_segmentation = await asyncio.gather(
                ...
                plot_threads_table = PrettyTable(
                    [
                        "name",
                        "chapter_started",
                        "chapter_ended",
                        "chapter_last_touched",
                        "status",
                    ]
                )
                ...
                acts_table = PrettyTable(
                    ["number", "chapter_started", "chapter_ended", "current_chapter"]
                )
            ...
            case "structure":
                ...
                tension_curve_table = PrettyTable(
                    [
                        "chapter_id",
                        "chapter_number",
                        "avg_tension",
                ...
                for tension_row in tension_and_pacing_curves.tension_curve:
                    tension_curve_table.add_row(
                        [
                            tension_row.chapter_id,
                            tension_row.chapter_number,
                            tension_row.avg_tension,
                            tension_row.scene_count,
                            tension_row.word_count,
                        ]
                    )

                pacing_curve_table = PrettyTable(
                    [
                        "chapter_id",
                        "chapter_number",
                        "avg_pacing",
                ...
                for pacing_row in tension_and_pacing_curves.pacing_curve:
                    pacing_curve_table.add_row(
                        [
                            pacing_row.chapter_id,
                            pacing_row.chapter_number,
                            pacing_row.avg_pacing,
                            pacing_row.scene_count,
                            pacing_row.word_count,
                        ]
                    )

                scene_length_distribution_table = PrettyTable(["bin", "count"])
                for length_row in scene_length_distribution.distribution:
                    scene_length_distribution_table.add_row(
                        [length_row.bin, length_row.count]
                    )

                recent_rythm_table = PrettyTable(
                    [
                        "chapter_id",
                        "chapter_number",
                        "avg_tension",
                ...
                for tension_row, pacing_row in zip(
                    recent_chapter_rythm.tension_curve,
                    recent_chapter_rythm.pacing_curve,
                ):
                    recent_rythm_table.add_row(
                        [
                            tension_row.chapter_id,
                            tension_row.chapter_number,
                            tension_row.avg_tension,
            ...
            case "world":
                entity_ledger, contradictions = await asyncio.gather(
                ...
                entity_ledger_table = PrettyTable(
                    ["type", "name", "chapter_first_appeared", "chapter_last_touched"]
                )
                for entity in entity_ledger.extraction.entities:
                    entity_ledger_table.add_row(
                        [
                            entity.type,
                            entity.name,
                            entity.chapter_first_appeared,
                            entity.chapter_last_touched,
                        ]
                    )

                contradictions_table = PrettyTable(
                    ["headline", "report", "relevant_chapters"]
                )
                for contradiction in contradictions.extraction.contradictions:
                    chapters = ", ".join(
                        [str(chapter) for chapter in contradiction.relevant_chapters]
                    )
                    contradictions_table.add_row(
                        [contradiction.headline, contradiction.report, chapters]
                    )
            ...
            case "questions":
                ...
                questions_raised = await self._analytics_repo.get_questions_raised_by_chapter(story_id, user_id)

                questions_raised_table = PrettyTable(
                    ["chapter_id", "chapter_number", "question"]
                )

                for question_row in questions_raised:
                    questions_raised_table.add_row([*question_row])

                return {
                    "questions_raised": questions_raised_table.get_string()
                }
    ...
    async def get_analytics_suggestion(
        ...
        if prompt_template is None:
            raise ServiceError(
                "Invalid lense! Must be 'character', 'plot', 'structure', or 'world'."
            )
        ...
        await self._cache.set(
            cache_key, response.model_dump_json(), ex=timedelta(hours=1)
        )
    ...
    async def get_cast_statistics(
        ...
        return CastStatisticsResponse(
            ...
            statistics=[
                CastStatisticsRow(
                    character=row[0], scene_count=row[1], word_count=row[2]
                )
                for row in rows
            ],
        )
    ...
    async def get_co_occurence_statistics(
        ...
        rows = await self._analytics_repo.get_character_co_occurence_statistics(
            story_id, user_id
        )
    ...
    async def get_scene_length_distribution(
        ...
        rows = await self._analytics_repo.get_scene_length_distribution(
            story_id, user_id
        )
        ...
        return SceneLengthDistributionResponse(
            ...
            story_title=story.title,
            distribution=[
                SceneLengthDistributionRow(bin=row[0], count=row[1]) for row in rows
            ],
        )
    ...
    async def get_tension_and_pacing_curves(
        ...
        rows = await self._analytics_repo.get_tension_and_pacing_curves(
            story_id, user_id
        )
    ...
    async def extract_plot_threads(
        self, story_id: str, user_id: str, ignore_cache: bool = False
    ) -> PlotThreadsResponse:
        story = await self._story_repo.get(story_id, user_id)
    ...
    async def get_world_dashboard(
        ...
        contradictions, entities = await asyncio.gather(
        ...
        return WorldDashboardResponse(
            suggestion=suggestion, contradictions=contradictions, entities=entities
        )
