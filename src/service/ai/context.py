    ...
    story_with_summaries = await (
        Story
            ...
            .prefetch_related(
                Prefetch(
                    "summaries",
                    queryset=Summary.filter(type=summary_type)
                )
            )
            .first()
    )
...
async def get_story_context(story_id: str) -> str:
    ...
    summary_types = [SummaryType.CHARACTER, SummaryType.PLOT, SummaryType.STYLE, SummaryType.WORLD]

    results = await asyncio.gather(
        *(get_context_by_type(summary_type, story_id) for summary_type in summary_types),
