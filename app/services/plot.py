...
class PlotService:
    ...
    async def get_all_unanswered_story_questions(self, user_id: str, story_id: str) -> StoryQuestionsResponse:     
        ...
        answered_questions_cursor = await self.mongodb.plot_extractions.aggregate([
            {"$match": {"story_id": story_id, "user_id": user_id}},
            {"$unwind": "$questions"},
    ...
    async def get_all_setups_with_no_payoffs(self, user_id: str, story_id: str) -> SetupResponse:
        ...
        setups_cursor = await self.mongodb.plot_extractions.aggregate([
            match,
            {"$unwind": "$setups"},
        ...
        payoffs_cursor = await self.mongodb.plot_extractions.aggregate([
            match,
            {"$unwind": "$payoffs"},
